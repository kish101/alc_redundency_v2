...
void set_shift_regs_data_width()
{
  ...
  sr.setNoOfShiftRegisters(numberOfShiftRegisters);
  DATA_WIDTH = (numberOfShiftRegisters * 8);
  //Serial.print("total IOs (DATA_WIDTH): ");
...
void exor_state_arr_check()
{
  ...
  // changes in expansion board/s
  if (exor_len != DATA_WIDTH)
  {
    // if expansion board/s added
    if (exor_len <= DATA_WIDTH)
    {
    ...
    // if expansion board/s removed
    else if (exor_len >= DATA_WIDTH)
    {
      exor_state_str = exor_state_str.substring(0, DATA_WIDTH );
    }//else if
  }//if
